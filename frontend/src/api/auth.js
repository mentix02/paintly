...
const getAuthToken = (email, password) => {
  ...
  return axios.post(`${BASE_URL}/token/`, authData).then(retData).catch(retError);
};
...
const registerBuyer = (name, email, password) => {
  ...
  return axios.post(`${BASE_URL}/register/`, authData).then(retData).catch(retError);
};
...
const sendResetPasswordLink = email => {
  ...
  return axios.post(`${BASE_URL}/forget/`, emailData).then(retData).catch(retError);
};
...
const resetPassword = (token, password) => {
  ...
  return axios.post(`${BASE_URL}/reset/`, resetPasswordData).then(retData).catch(retError);
};
...
const validateResetToken = token => {
  ...
  return axios.post(`${BASE_URL}/validate/`, tokenData).then(retData).catch(retError);
};
