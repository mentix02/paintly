...
function Cart() {
  ...
  }, [setTotal, setMessage, setLoading, setVisible, setCartItems]);

  const sumReduceFunc = (prev, cartItem) =>
    prev + cartItem.quantity * cartItem.painting.price;
  ...
  const handleRemoveButtonClick = function (cart_item_id) {
    removeCartItem(cart_item_id)
      .then((_) => {
        ...
        const rmIndex = cartItemsCopy
          .map((item) => item.id)
          .indexOf(cart_item_id);
  ...
  const handleDecrementButtonClick = function (cart_item_id) {
    decrementCartItemQuantity(cart_item_id).then((_) => {
      const cartItemsCopy = JSON.parse(JSON.stringify(cartItems));
      const decrementIndex = cartItemsCopy
        .map((item) => item.id)
        .indexOf(cart_item_id);
                    ...
                    <tr key={index}>
                      ...
                      <td>
                        <ButtonGroup>
                          <Button
                            onClick={() =>
                              handleDecrementButtonClick(cartItem.id)
                            }
                            size="sm"
                            color="warning"
                          >
                          ...
                          <Button
                            onClick={() => handleRemoveButtonClick(cartItem.id)}
                            size="sm"
                            color="danger"
                          >
                            <BsTrashFill />
                          ...
                          <Button
                            onClick={() =>
                              handleIncrementButtonClick(cartItem.id)
                            }
                            size="sm"
                            color="success"
                          >
