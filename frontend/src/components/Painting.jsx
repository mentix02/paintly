...
import { FaAngleUp, FaCartPlus, FaAngleDown } from "react-icons/fa";
import {
  Card,
  Button,
  CardImg,
...
function Painting(props) {
  ...
  const getTagsFromSearch = useCallback((url) => {
    ...
    if (tempTags.length === 1 && tempTags[0].length === 0) return [];
    else return tempTags;
  }, []);
  ...
  function appendBadgeLink(tag) {
    ...
    if (tags.length === 0) return `${location.pathname}?tags=${tag}`;
    else return `${location.pathname}?tags=${prevTags.join(",")},${tag}`;
  }
  ...
  return (
    <Card>
      ...
      <CardBody>
        ...
        <CardSubtitle>
          <h6 className="mt-4">
            {painting.height}x{painting.width} inches at ₹
            {toCurrency(painting.price)}
          </h6>
          <ButtonGroup>
            {painting.tags.map((tag, index) => (
              <DisabledLink
                disabled={tags.includes(tag)}
                to={appendBadgeLink(tag)}
